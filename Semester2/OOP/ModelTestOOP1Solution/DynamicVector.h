...
int DynamicVector<T>::equal(T v, int pos)
{
	...
	if (this->elems[pos].getName() == v.getName() && this->elems[pos].getTeam() == v.getTeam() && this->elems[pos].getNationality() == v.getNationality() && this->elems[pos].getGoals() == v.getGoals())
		return 1;
	return 0;
}
...
template <typename T>
int DynamicVector<T>::update(int position,T e)
{
