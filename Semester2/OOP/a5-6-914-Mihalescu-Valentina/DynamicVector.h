...
public:
	DynamicVector(int capacity = 1); //the normal constructor
	DynamicVector(const DynamicVector&); //copy constructor->dynamically allocated memory

	DynamicVector& operator=(const DynamicVector&); //when we have a manually defined copy constructor we also need a manually defined = operator
