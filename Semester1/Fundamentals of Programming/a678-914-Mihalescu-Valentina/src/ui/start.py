...
class Console:
    ...
    def execute_user_command(self, option):
        ...
        if int(option) == 1:
            ...
            grades = list(filter(lambda x: x.student_id == stud_id, self.__grade_service.get_grades))
            ...
            grades = list(filter(lambda x: x.assignment_id == assignment_id, self.__grade_service.get_grades))
